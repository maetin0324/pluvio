use std::{cell::RefCell, mem::ManuallyDrop, os::unix::io::RawFd, rc::Rc};
...
impl FixedBufferAllocator {
    ...
    pub fn grow(&mut self, ring: &mut IoUring) {
        ...
        for _ in 0..current_len {
            ...
            self.buffers
                .push(RefCell::new(ManuallyDrop::new(buf)));
        }
        // Build iovecs from the preallocated buffers.
        let iovecs: Vec<iovec> = self.buffers
            .iter()
            .map(|buf_mutex| {
                let buf = buf_mutex.borrow_mut();
                iovec {
...
impl Drop for FixedBufferAllocator {
    fn drop(&mut self) {
        ...
        for buf in self.buffers.iter() {
            unsafe {
                ManuallyDrop::drop(&mut *buf.borrow_mut());
            }
        }
    }
}
